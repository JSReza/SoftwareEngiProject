...
import main.java.ConceptualAPI;
import main.java.ConceptualAPIPrototype;
import main.java.NetworkAPI;
import main.java.NetworkAPIPrototype;
import main.java.ProcessAPI;
import main.java.ProcessAPIPrototype;

/**
 * This test checks that all 3 APIs exist in the 'src' folder as interfaces,
 * with the appropriate annotations, and that they all
 ...
 * the APIs - use Eclipse to auto-generate classes,
 * interfaces, etc as you build out the prototype. This ensures that your API is
 * written from the perspective of the client - the prototype
 * acts as a "pretend" client; while the prototype code will never actually run,
 * it's a bare-bones check that some other fully-featured client
...
public class Assignment2StructureTest {
	...
	public void checkPrototypesExist(Class<? extends Annotation> apiAnnotation,
		...
		if (numPrototypesFound != 1) {
			errors.add("No (or multiple) class method with the " + prototypeAnnotation.getSimpleName()
	...
	public void checkAnnotationsExist(Class<? extends Annotation> apiAnnotation) throws Exception {
		...
		if (numApisFound != 1) {
			errors.add(
					"No (or multiple) interface found with annotation " + apiAnnotation.getSimpleName() + " in 'src'");
